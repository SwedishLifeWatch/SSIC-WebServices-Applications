...
{
    using RedListCalculation = ArtDatabanken.Data.ArtDatabankenService.RedListCalculation;

    [TestClass]
    public class RedListCalculationTest : TestBase
    {
        private const Int64 AREA_OF_OCCUPANCY_TEST_MAX = 5000;
        private const Int64 AREA_OF_OCCUPANCY_TEST_INCREASE = 100;
        private const Int64 EXTENT_OF_OCCURRENCE_TEST_MAX = 50000;
        private const Int64 EXTENT_OF_OCCURRENCE_TEST_INCREASE = 100;
        private const Double MAX_PROPORTION_LOCAL_POPULATION_TEST_MIN = 85;
        private const Int64 MAX_SIZE_LOCAL_POPULATION_TEST_MAX = 2100;
        private const Int64 MAX_SIZE_LOCAL_POPULATION_TEST_INCREASE = 50;
        private const Int64 NUMBER_OF_LOCATIONS_TEST_MAX = 50;
        private const Double POPULATION_REDUCTION_A1_TEST_MIN = -10;
        private const Double POPULATION_REDUCTION_A2_TEST_MIN = -10;
        private const Double POPULATION_REDUCTION_A3_TEST_MIN = -10;
        private const Double POPULATION_REDUCTION_A4_TEST_MIN = -10;
        private const Int64 POPULATION_SIZE_TEST_MAX = 25000;
        ...
        public void AreaOfOccupancy()
        {
            ...
            for (areaOfOccupancy = RedListCalculation.AREA_OF_OCCUPANCY_MIN; areaOfOccupancy <= AREA_OF_OCCUPANCY_TEST_MAX; areaOfOccupancy += AREA_OF_OCCUPANCY_TEST_INCREASE)
            {
        ...
        public void Category()
        {
            ...
            CategoryTestC(RedListCalculation.POPULATION_SIZE_MIN + 1,
                          ...
                          MAX_PROPORTION_LOCAL_POPULATION_TEST_MIN,
            ...
            CategoryTestC(RedListCalculation.CRITERIA_C_EN_LIMIT,
                          ...
                          MAX_SIZE_LOCAL_POPULATION_TEST_MAX,
                          RedListCalculation.CRITERIA_C2A2_CR_LIMIT,
                          RedListCalculation.CRITERIA_C2A2_NT_LIMIT - 1,
                          RedListCategory.NT);
            CategoryTestC(RedListCalculation.CRITERIA_C_EN_LIMIT,
                                   ...
                                   RedListCategory category)
        {
            ...
            for (areaOfOccupancy = areaOfOccupancyMin; areaOfOccupancy <= areaOfOccupancyMax; areaOfOccupancy += AREA_OF_OCCUPANCY_TEST_INCREASE)
            {
                for (extentOfOccurrence = extentOfOccurrenceMin; extentOfOccurrence <= extentOfOccurrenceMax; extentOfOccurrence += EXTENT_OF_OCCURRENCE_TEST_INCREASE)
                {
                                   ...
                                   RedListCategory category)
        {
            ...
            for (populationSize = populationSizeMin; populationSize <= populationSizeMax; populationSize += POPULATION_SIZE_TEST_INCREASE)
            {
                for (continuingDecline = continuingDeclineMin; continuingDecline <= continuingDeclineMax; continuingDecline++)
                {
                    for (maxSizeLocalPopulation = maxSizeLocalPopulationMin; maxSizeLocalPopulation <= maxSizeLocalPopulationMax; maxSizeLocalPopulation += MAX_SIZE_LOCAL_POPULATION_TEST_INCREASE)
                    {
        ...
        public void Criteria()
        {
            ...
            GetRedListCalculation().ContinuingDecline = 1;
            ...
            GetRedListCalculation(true).NumberOfLocations = 4;
            //            Assert.AreEqual("D2", GetRedListCalculation().Criteria);

            GetRedListCalculation(true).ProbabilityOfExtinction = 4;
        ...
        public void ExtentOfOccurrence()
        {
            ...
            // Test value.
            GetRedListCalculation(true);
            for (extentOfOccurrence = RedListCalculation.EXTENT_OF_OCCURRENCE_MIN; extentOfOccurrence <= EXTENT_OF_OCCURRENCE_TEST_MAX; extentOfOccurrence += EXTENT_OF_OCCURRENCE_TEST_INCREASE)
            {
                GetRedListCalculation().ExtentOfOccurrence = extentOfOccurrence;
        ...
        public void GetCriteriaBCount()
        {
            ...
            for (severlyFragmented = GetRedListCalculation().SeverlyFragmentedMin; severlyFragmented <= GetRedListCalculation().SeverlyFragmentedMax; severlyFragmented++)
            {
                for (continuingDecline = GetRedListCalculation().ContinuingDeclineMin; continuingDecline <= GetRedListCalculation().ContinuingDeclineMax; continuingDecline++)
                {
                    for (extremeFluctuations = GetRedListCalculation().ExtremeFluctuationsMin; extremeFluctuations <= GetRedListCalculation().ExtremeFluctuationsMax; extremeFluctuations++)
                    {
                        for (numberOfLocations = RedListCalculation.NUMBER_OF_LOCATIONS_MIN; numberOfLocations <= NUMBER_OF_LOCATIONS_TEST_MAX; numberOfLocations++)
                        {
                            criteriaBCount = 0;
        ...
        private RedListCalculation GetRedListCalculation()
        {
            return GetRedListCalculation(false);
        }

        private RedListCalculation GetRedListCalculation(Boolean refresh)
        {
            if (_redListCalculation.IsNull() || refresh)
            {
                _redListCalculation = new RedListCalculation();
        ...
        {
            /*           // Test value.
                       GetRedListCalculation(true).IsConservationDependent = false;
                       Assert.IsFalse(GetRedListCalculation().IsConservationDependent);
                       GetRedListCalculation().IsConservationDependent = true;
                       Assert.IsTrue(GetRedListCalculation().IsConservationDependent);

                       // Test category.
                       GetRedListCalculation(true).IsConservationDependent = false;
                       Assert.AreEqual(RedListCategoryId.LC, GetRedListCalculation().Category);
                       GetRedListCalculation().IsConservationDependent = true;
                       Assert.AreEqual(RedListCategoryId.NT, GetRedListCalculation().Category);
        ...
        public void IsCriteriaA1AFulfilled()
        {
            ...
            // Test category.
            GetRedListCalculation(true);
            for (populationReduction = POPULATION_REDUCTION_A1_TEST_MIN; populationReduction <= RedListCalculation.POPULATION_REDUCTION_A1_MAX; populationReduction++)
            {
        ...
        public void IsCriteriaA2AFulfilled()
        {
            ...
            // Test category.
            GetRedListCalculation(true);
            for (populationReduction = POPULATION_REDUCTION_A2_TEST_MIN; populationReduction <= RedListCalculation.POPULATION_REDUCTION_A2_MAX; populationReduction++)
            {
        ...
        public void IsCriteriaA3BFulfilled()
        {
            ...
            // Test category.
            GetRedListCalculation(true);
            for (populationReduction = POPULATION_REDUCTION_A3_TEST_MIN; populationReduction <= RedListCalculation.POPULATION_REDUCTION_A3_MAX; populationReduction++)
            {
        ...
        public void IsCriteriaA4AFulfilled()
        {
            ...
            // Test category.
            GetRedListCalculation(true);
            for (populationReduction = POPULATION_REDUCTION_A4_TEST_MIN; populationReduction <= RedListCalculation.POPULATION_REDUCTION_A4_MAX; populationReduction++)
            {
        ...
        public void IsCriteriaBB1Fulfilled()
        {
            ...
            // Test category.
            for (areaOfOccupancy = RedListCalculation.AREA_OF_OCCUPANCY_MIN + 1; areaOfOccupancy <= AREA_OF_OCCUPANCY_TEST_MAX; areaOfOccupancy += AREA_OF_OCCUPANCY_TEST_INCREASE)
            {
        ...
        {
            Boolean isCriteriaCalculated;

            // Test value.
            isCriteriaCalculated = true;
            GetRedListCalculation(true).IsCriteriaCalculated = isCriteriaCalculated;
            Assert.AreEqual(isCriteriaCalculated, GetRedListCalculation().IsCriteriaCalculated);
            isCriteriaCalculated = false;
            GetRedListCalculation().IsCriteriaCalculated = isCriteriaCalculated;
            Assert.AreEqual(isCriteriaCalculated, GetRedListCalculation().IsCriteriaCalculated);
        ...
        public void MaxSizeLocalPopulation()
        {
            ...
            // Test value.
            GetRedListCalculation(true);
            for (maxSizeLocalPopulation = RedListCalculation.MAX_SIZE_LOCAL_POPULATION_MIN; maxSizeLocalPopulation <= MAX_SIZE_LOCAL_POPULATION_TEST_MAX; maxSizeLocalPopulation += MAX_SIZE_LOCAL_POPULATION_TEST_INCREASE)
            {
                GetRedListCalculation().MaxSizeLocalPopulation = maxSizeLocalPopulation;
        ...
        public void PopulationSize()
        {
            ...
            // Test value.
            GetRedListCalculation(true);
            for (populationSize = RedListCalculation.POPULATION_SIZE_MIN; populationSize <= POPULATION_SIZE_TEST_MAX; populationSize += POPULATION_SIZE_TEST_INCREASE)
            {
