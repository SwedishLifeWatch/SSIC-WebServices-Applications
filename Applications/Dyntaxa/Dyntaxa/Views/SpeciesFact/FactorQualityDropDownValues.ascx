...
<% if(Model.IsHost && ((int)Model.MainParentFactorId == (int)DyntaxaFactorId.SUBSTRATE)) { %>
    <% string val = "";
        foreach (var fieldValue in Model.QualityValues.QualityValues)
       {



           if (fieldValue.Key == Model.QualityId)
           {
...
<% } 
   ...
   { %>
    <select <% if(!Model.IsOkToUpdate) { %>  disabled="disabled" <% } %> name="<%=string.Format("factorEnumFieldValue3_{0}",Model.FactorId + "_" + Model.HostId + "_" + Model.IndividualCategoryId + "_" + Model.ReferenceId + "_" + Model.QualityId)%>">      
    <%     
